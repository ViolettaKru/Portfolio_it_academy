...
// неохраняемый в другом месте, кажется безопаснее определять глобальный только для этого модуля
            ...
            if ((options = arguments [i])! = null) {
                ...
                для (имя в опциях) {
                    ...
                    if (deep && copy && (jQuery.isPlainObject (copy) ||
                        ...
                        if (copyIsArray) {
                            copyIsArray = false;
                            clone = src && Array.isArray (src)? источник: [];

                        } еще {
                            clone = src && jQuery.isPlainObject (src)? источник: {};
                        }
    ...
    map: function (elems, callback, arg) {
        ...
        // Сглаживать любые вложенные массивы
...
// Заполняем карту class2type
    ...
    function isArrayLike (obj) {
        ...
        typeof длина === "число" && длина> 0 && (длина - 1) в объекте;
    }
    var Sizzle =
        / *!
...
") \\) |)",
    ...
    rattributeQuotes = new RegExp ("=" + пробел + "* ([^ \\] '\"] *?) "+ пробел +" * \\] "," g "),
...
rinputs = / ^ (?: input | select | textarea | button) $ / i,
    ...
    funescape = function (_, сбежал, escapedWhitespace) {
        ...
        возвращайся высоко! == высоко || побег пробел?
            сбежал:
            высокий <0?
                // код BMP
    ...
    fcssescape = function (ch, asCodePoint) {
        if (asCodePoint) {
            ...
            // U + 0000 NULL становится U + FFFD ЗАМЕНЯЮЩИЙ ХАРАКТЕР
            if (ch === "\ 0") {
                return "\ uFFFD";
            }
    ...
    disabledAncestor = addCombinator (
        function (elem) {
            return elem.disabled === true && ("форма" в элементе || "метка" в элементе);
        },
        ...
        if (documentIsHTML) {
            ...
            if (support.qsa &&
                ...
                if (nodeType! == 1) {
                    ...
                    // qSA смотрит вне контекста элемента, а это не то, что нам нужно
                    // Спасибо Эндрю Дюпон за эту технику обхода
                    // Поддержка: IE <= 8
...
функция createDisabledPseudo (отключена) {
    ...
    функция возврата (элемент) {
        ...
        if ("форма" в элементе) {
            ...
            // Проверяем унаследованную инвалидность на соответствующих неотключенных элементах:
            ...
            if (elem.parentNode && elem.disabled === false) {
                ...
                // Поддержка: IE 6 - 11
                // Использование свойства ярлыка isDisabled для проверки отключенных предков fieldset
                return elem.isDisabled === отключено ||

                    // Там, где нет isDisabled, проверить вручную
                    / * jshint -W018 * /
                elem.isDisabled! ==! отключен &&
                disabledAncestor (elem) === отключено;
            }
...
isXML = Sizzle.isXML = function (elem) {
    ...
    // (например, загрузка фреймов в IE - # 4833)
    var documentElement = elem && (elem.ownerDocument || elem) .documentElement;
    вернуть документЭлемент? documentElement.nodeName! == "HTML": false;
};
...
setDocument = Sizzle.setDocument = function (node) {
    var hasCompare, subWindow,
        документ = узел? node.ownerDocument || узел: предпочитаемый док;
...
функция (а, б) {
    ...
    // Если узлы братья и сестры, мы можем сделать быструю проверку
    } else if (aup === bup) {
    ...
    вернуть я?
        // Проверка родства, если у узлов есть общий предок
...
/ **
* Утилита для извлечения текстового значения из массива узлов DOM
...
Предварительный фильтр: {
    "ATTR": функция (соответствие) {
        ...
        // Переместить указанное значение в match [3], в кавычках или без кавычек
    ...
    "PSEUDO": function (match) {
        ...
        // Удалить лишние символы из аргументов без кавычек
                        ...
                        if (diff === false) {
                            ...
                            while ((node ​​= ++ nodeIndex && node && node [dir] ||
                                ...
                                if ((ofType?
                                    ...
                                    // Кешируем индекс каждого найденного элемента
                                    if (useCache) {
                                        externalCache = узел [раскрыть] || (узел [раскрыть] = {});
...
псевдо: {
    ...
    // или начинающийся с идентификатора C, за которым сразу следует "-".
...
return markFunction (function (seed, results, context, xml) {
    ...
    if (postFilter) {
        temp = конденсат (matcherOut, postMap);
    ...
    if (seed) {
        if (postFinder || preFilter) {
            if (postFinder) {
                // Получить последний matcherOut, сжимая это промежуточное звено в контексты postFinder
                temp = [];
                я = matcherOut.length;
                в то время как я-- ) {
...
var checkContext, matcher, j,
    ...
    // Основополагающий механизм сопоставления обеспечивает доступность элементов из контекста (ов) верхнего уровня
...
var bySet = setMatchers.length> 0,
    ...
    superMatcher = function (seed, context, xml, results, external) {
        ...
        for (; i! == len && (elem = elems [i])! = null; i ++) {
            ...
            // Отслеживание несоответствующих элементов для установленных фильтров
            if (bySet) {
        ...
        // `i` теперь подсчитывает количество посещенных элементов и добавляет его в` matchedCount`
        // делает последний неотрицательным.
        matchedCount + = i;

        // Применяем набор фильтров к несогласованным элементам
        // ПРИМЕЧАНИЕ: это можно пропустить, если нет несопоставленных элементов (например, `matchedCount`
        // равно `i`), если мы не посетили _any_ элементы в вышеуказанном цикле, потому что у нас есть
        ...
        if (bySet && i! == matchedCount) {
            ...
            if (seed) {
                ...
                // Сбрасываем значения заполнителя индекса, чтобы получить только фактические совпадения
...
// Попытка минимизировать операции, если в списке только один селектор и нет начального числа
...
if (match.length === 1) {
    ...
    if (tokens.length> 2 && (token = tokens [0]). type === "ID" &&
        ...
        // Предварительно скомпилированные сопоставители по-прежнему проверяют происхождение, поэтому повышайте уровень
...
// Поддержка: IE <9
// Использование getAttributeNode для получения логических значений, когда getAttribute лежит
if (! assert (function (el) {
...
// массив элементов (jQuery, arguments, Array)
...
var rparentsprev = / ^ (?: родители | пред (?: до | всех)) /,
...
index: function (elem) {
    ...
    // Определить местоположение нужного элемента
...
prevUntil: функция (элемент, я, пока) {
    return dir (elem, «предыдущий брат», пока);
},
...
* Возможные варианты:
*
* Once: гарантирует, что список обратных вызовов может быть запущен только один раз (например, Deferred).
*
...
var // Флаг, чтобы знать, если список в настоящее время срабатывает
    стрельба,
    ...
    огонь = функция () {
        ...
        // Принудительное однократное срабатывание
...
Deferred: function (func) {
    var tuples = [
            ...
            ["решить", "сделано", jQuery.Callbacks ("однажды память"),
                jQuery.Callbacks («однажды память»), 0, «разрешен»],
            ...
            pipe: function (/ * fnDone, fnFail, fnProgress * /) {
                ...
                return jQuery.Deferred (function (newDefer) {
                    jQuery.each (tuples, function (i, tuple) {
                        ...
                        // Сопоставим кортежи (прогресс, выполнено, неудача) с аргументами (выполнено, неудача, прогресс)
                        ...
                        вар это = это,
                            ...
                            mightThrow = function () {
                                ...
                                if (return === deferred.promise ()) {
                                    бросить новую TypeError («Собственное разрешение»);
                                }
                                ...
                                тогда = вернул &&
                                    ...
                                    // Проверяем только объекты и функции на предмет возможности
                                ...
                                if (isFunction (then)) {
                                    ...
                                    // Обычные процессоры (решают) также подключаются к прогрессу
                                    } еще {
                                ...
                                // Обрабатываем все остальные возвращаемые значения
                                    ...
                                    // и несколько значений (не специфика поведения)
                                    if (handler! == Identity) {
                                        это = не определено;
                                        args = [возвращено];
                                    }
...
когда: функция (singleValue) {
    ...
    если (осталось <= 1) {
        ...
        // Используйте .then () для разворачивания вторичных объектов (см. Gh-3000)
...
// Обрабатываем это асинхронно, чтобы дать возможность сценариям задержать готовность
    ...
    if (навальный) {
        ...
        // ... кроме случаев выполнения значений функций
            ...
            fn = функция (элемент, ключ, значение) {
                вернуть bulk.call (jQuery (elem), значение);
            };
        }
    }
...
кеш: функция (владелец) {
    ...
    if (! value) {
        ...
        if (acceptData (owner)) {
            ...
            // Если это узел, который вряд ли будет зашифрован или зациклен на
...
set: function (владелец, данные, значение) {
    ...
    // Обработка: [owner, {properties}] args
        ...
        // Копируем свойства один за другим в объект кеша
        for (опора в данных) {
...
// 2. Улучшаем ремонтопригодность модуля, уменьшая объем хранилища
...
// TODO: теперь, когда все вызовы _data и _removeData были заменены
...
вар регулируется, масштаб,
    ...
    // Начальное вычисление значения требуется для потенциальных несоответствий единиц
...
if (initialInUnit && initialInUnit [3]! == единица измерения) {
    ...
    // Трастовые единицы, о которых сообщает jQuery.css
    ...
    while (maxIterations--) {
        ...
        if ((1 - шкала) * (1 - (шкала = currentValue () / начальная || 0,5)) <= 0) {
            maxIterations = 0;
        }
...
for (; index <длина; index ++) {
    ...
    если (показать) {
        ...
        // Так как мы навязываем скрытые каскадом элементы, немедленное (и медленное)
...
// Установить отображение элементов во втором цикле, чтобы избежать постоянного перекомпоновки
...
// Парсеры XHTML магически не вставляют элементы в
...
// Помечаем скрипты как уже оцененные
...
var elem, tmp, tag, wrap, содержит, j,
...
while ((elem = node [i ++])) {
    ...
    has = jQuery.contains (elem.ownerDocument, elem);
...
// Поддержка: только IE <= 9
// Смотрите # 13393 для получения дополнительной информации
...
функция включена (элемент, типы, селектор, данные, fn, one) {
var origFn, тип;
...
добавить: функция (элемент, типы, обработчик, данные, селектор) {
    ...
    // Оцениваем по documentElement в случае, если elem является неэлементным узлом (например, document)
    if (селектор) {
    ...
    while (t--) {
        ...
        // Обновление специального на основе нового типа сброса
        special = jQuery.event.special [тип] || {};
...
dispatch: function (nativeEvent) {
    ...
    // Создаем доступный для записи jQuery.Event из собственного объекта события
    ...
    // Вызовите ловушку preDispatch для сопоставленного типа и, если необходимо, разрешите его сброс
    ...
    while ((matched = handlerQueue [i ++]) &&! event.isPropagationStopped ()) {
        ...
        while ((handleObj = matched.handlers [j ++]) &&
            ...
            // Инициированное событие должно либо: 1) не иметь пространства имен, либо 2) иметь пространство (ы) имен
            // подмножество или равное ему в связанном событии (оба не могут иметь пространства имен).
    ...
    // Вызовите ловушку postDispatch для сопоставленного типа
...
специальный: {
    загрузить: {
        ...
        // Предотвращаем инициируемые события image.load от всплывающих окон в window.load
        noBubble: правда
    },
    фокус: {

        // Запускаем собственное событие, если возможно, чтобы последовательность размытия / фокусировки была правильной
        триггер: функция () {
    ...
    нажмите: {
        ...
        // Для флажка запускаем собственное событие, поэтому проверенное состояние будет правильным
        триггер: функция () {
        ...
        // Для согласованности между браузерами не запускайте native .click () для ссылок
...
// Включает все общие события, включая KeyEvent и MouseEvent.
jQuery.each ({
altKey: правда,
пузыри: правда,
...
// Создание указателя мыши / выхода из событий с помощью проверок при наведении курсора / выходе и времени события
...
jQuery.event.special [orig] = {
    ...
    handle: function (event) {
        ...
        // Для мышки / оставьте вызов обработчика, если связанный находится за пределами цели.
...
// 1. Скопировать личные данные: события, обработчики и т. Д.
            ...
            for (i = 0; i <hasScripts; i ++) {
                ...
                if (rscriptType.test (node.type || "") &&
                    ...
                    if (node.src && (node.type || "") .toLowerCase ()! == "module") {
                        ...
                        // Необязательная зависимость AJAX, но не запускает сценарии, если не присутствует
                        if (jQuery._evalUrl) {
...
clone: ​​function (elem, dataAndEvents, deepDataAndEvents) {
    ...
    // Устранить проблемы с клонированием IE
    if (! support.noCloneChecked && (elem.nodeType === 1 || elem.nodeType === 11) &&
            ! jQuery.isXMLDoc (elem)) {

        // Мы избегаем Sizzle здесь из соображений производительности: https://jsperf.com/getall-vs-sizzle/2
...
jQuery.fn [имя] = функция (селектор) {
    ...
    for (; i <= last; i ++) {
        elems = я === последний? this: this.clone (правда);
...
вычисляется = вычисляется || getStyles (elem);
...
// Ярлык для имен без префикса поставщика
if (имя в emptyStyle) {
    вернуть имя;
}
...
// Возвращаем свойство, сопоставленное с тем, что предлагает jQuery.cssProps или
// свойство с префиксом поставщика.
function finalPropName (name) {
var ret = jQuery.cssProps [имя];
if (! ret) {
    ret = jQuery.cssProps [name] = vendorPropName (name) || название;
}
возвратный ответ;
}
...
// Проверка стиля в случае, если браузер возвращает ненадежные значения
// для getComputedStyle молча возвращается к надежному elem.style
...
if (val === "auto" ||
    ...
    // offsetWidth / offsetHeight предоставляет значения рамки
...
// Добавить в свойства, имена которых вы хотите исправить раньше
...
jQuery.cssHooks [измерение] = {
    get: function (elem, computed, extra) {
        if (вычислено) {
            ...
            // но у него должен быть текущий стиль отображения, который будет полезен
...
jQuery.cssHooks [префикс + суффикс] = {
    развернуть: функция (значение) {
        var i = 0,
            ...
            части = значение typeof === "строка"? value.split (""): [значение];
...
вернуть новый Tween.prototype.init (элемент, опции, проп, конец, ослабление);
}
jQuery.Tween = Tween;
...
для (опора в реквизит) {
    ...
    if (rfxtypes.test (value)) {
        ...
        if (value === (hidden? "hide": "show")) {
            ...
            // Игнорируем все остальные неоперативные показ / скрытие данных
            } еще {
                Продолжить;
            }
        }
...
// Реализация показать / скрыть анимацию
propTween = false;
for (опора в оригинале) {

    // Общая настройка показа / скрытия анимации этого элемента
    if (! propTween) {
        if (dataShow) {
...
fadeTo: function (скорость, до, замедление, обратный вызов) {
    ...
    вернуть this.filter (isHiddenWithinTree) .css ("непрозрачность", 0) .show ()
        ...
        // Анимируем до указанного значения
...
отделка: функция (тип) {
    ...
    вернуть this.each (function () {
        ...
        // Ищем анимации в старой очереди и заканчиваем их
        для (индекс = 0; индекс <длина; индекс ++) {
...
вернуть this.queue (тип, функция (следующая, перехватывает) {
...
set: function (elem, value, name) {
    if (value === false) {
        ...
        // Удаляем логические атрибуты при значении false
        jQuery.removeAttr (элемент, имя);
    } еще {
...
attrHandle [name] = function (elem, name, isXML) {
    ...
    if (! isXML) {
        ...
        handle = attrHandle [нижний регистр];
...
prop: function (elem, name, value) {
    ...
    // Не получаем / не устанавливаем свойства для узлов текста, комментариев и атрибутов
    if (nType === 3 || nType === 8 || nType === 2) {
        вернуть;
    }
...
// когда в оптгруппе
// правило eslint "no-unused-выражения" отключено для этого кода
    ...
    if (classes.length) {
        while ((elem = this [i ++])) {
            ...
            if (cur) {
                ...
                // Назначаем только если отличны, чтобы избежать ненужного рендеринга.
    ...
    вернуть this.each (function () {
        ...
        if (isValidValue) {
            ...
            while ((className = classNames [i ++])) {
                ...
                // Проверка каждого заданного className, разделенного пробелами списка
...
trigger: function (event, data, elem, onlyHandlers) {
    ...
    // Переходим к документу, затем к окну; следите за глобальным владельцемDocument var (# 9724)
    ...
    if (! onlyHandlers &&! event.isDefaultPrevented ()) {
        ...
        if ((! special._default ||
            ...
            // Вызвать собственный метод DOM для цели с тем же именем, что и у события.
...
// Билет по теме - https://bugzilla.mozilla.org/show_bug.cgi?id=687787
//
...
jQuery.each ({focus: "focusin", blur: "focusout"}, функция (orig, fix) {
    ...
    // Прикрепить один обработчик захвата к документу, пока кто-то хочет focusin / focusout
...
/ * Префильтры
 * 1) Они полезны для представления пользовательских типов данных (см. Ajax / jsonp.js для примера)
 * 2) Это называется:
...
// Избегаем комментария-пролога char sequence (# 10098); должен успокоить пух и уклониться от сжатия
allTypes = "* /". concat ("*"),
...
// Базовая функция контроля для предварительных фильтров и транспорта
...
// Удалить auto dataType и получить тип контента в процессе
while (dataTypes [0] === "*") {
    dataTypes.shift ();
    if (ct === undefined) {
...
while (текущий) {
    ...
    if (current) {
        ...
        // Преобразовать ответ, если предыдущий dataType не является автоматическим и отличается от текущего
...
ajax: function (url, options) {
    ...
    // Заставляем опции быть объектом
    опции = опции || {};
    ...
    вар транспорт,
        ...
        // Создаем конечный объект параметров
        ...
        jqXHR = {
            ...
            getResponseHeader: function (key) {
                ...
                возвращать совпадение == ноль? null: match;
            },
    ...
    s.type = options.method || options.type || с.метод || s.type;
    ...
    // Мы можем запускать глобальные события на данный момент, если их попросить
    ...
    // Обратный вызов для когда все сделано
    функция выполнена (статус, nativeStatusText, ответы, заголовки) {
        ...
        if (isSuccess) {
            ...
            if (s.ifModified) {
                ...
                если (изменено) {
                    jQuery.lastModified [cacheURL] = измененный;
                }
...
if (support.cors || xhrSupported &&! options.crossDomain) {
    вернуть {
        send: function (заголовки, завершено) {
            ...
            // (он всегда может быть установлен для каждого запроса или даже с использованием ajaxSetup)
            ...
            if (xhr.onabort! == undefined) {
                ...
                xhr.onreadystatechange = function () {
                    ...
                    if (xhr.readyState === 4) {
                        ...
                        // но это не будет обрабатывать собственный прерывание
            ...
            пытаться {
                ...
                // # 14683: Перебрасывать, только если об этом еще не было сообщено как ошибка
                if (callback) {
                    бросить е;
                }
            }
...
// Запретить автоматическое выполнение сценариев, если не был указан явный dataType (см. Gh-2432)
...
if (s.crossDomain) {
    ...
    вернуть {
        отправить: функция (_, завершено) {
            ...
            // Используем собственные манипуляции с DOM, чтобы избежать нашего трюка с domManip AJAX
...
// свернуть родные формы: второй становится потомком первого.
...
if (! context) {
    ...
    // Остановить немедленное выполнение скриптов или встроенных обработчиков событий
...
if (self.length> 0) {
    ...
    // В случае неудачи эта функция получает «jqXHR», «status», «error»
...
setOffset: function (elem, options, i) {
    ...
    // Сначала устанавливаем позицию, в случае, если верх / лев установлены даже на статический элемент
    if (position === "static") {
        elem.style.position = "родственник";
    }
    ...
    // Вверху или влево установлено значение «Авто», а позиция - абсолютная или фиксированная
    if (CalculayPosition) {
...
position: function () {
    ...
    // position: фиксированные элементы смещены относительно области просмотра, которая всегда имеет нулевое смещение
    if (jQuery.css (elem, "position") === "fixed") {
        ...
        if (offsetParent && offsetParent! == elem && offsetParent.nodeType === 1) {
            ...
            // Включить границы в его смещение, так как они находятся за пределами источника контента
...
//
// Однако эта логика не гарантируется и может измениться в любой момент в будущем
...
// Создание методов scrollLeft и scrollTop
jQuery.each ({scrollLeft: "pageXOffset", scrollTop: "pageYOffset"}, функция (метод, проп) {
var top = "pageYOffset" === prop;

jQuery.fn [method] = function (val) {
    вернуть доступ (this, функция (elem, method, val) {
...
// Добавляем верхний / левый cssHooks используя jQuery.fn.position
    ...
    jQuery.fn [funcName] = function (margin, value) {
        ...
        вернуть доступ (this, функция (elem, type, value) {
            ...
            if (isWindow (elem)) {
                ...
                // $ (window) .outerWidth / Height возвращаемое значение w / h, включая полосы прокрутки (gh-1729)
            ...
            if (elem.nodeType === 9) {
                ...
                вернуть Math.max (
                    ...
                    elem.body ["смещение" + имя], документ ["смещение" + имя],
                    doc ["клиент" + имя]
                );
            }
...
jQuery.each (("размытие фокуса в фокусе при изменении размера прокрутки нажмите dblclick" +
"mousedown mouseup mousemove mouseover mouseout Mouse Mouseleave" +
